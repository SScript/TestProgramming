...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
  ...
  protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
     ...
     *       / \                    / \
     *      t0  t1                 t1  t2
     * </pre> Caller should ensure that p is not the root.
     */
     ...
     * </pre> The subtree will be restructured so that the node with key b
     * becomes its root.
     * <pre>
     *           b
     *         /   \
     ...
     *      / \     / \
     *     t0  t1  t2  t3
     * </pre> Caller should ensure that x has a grandparent.
     */
   ...
   * returns the old value. Otherwise, a new entry is added and null is
   * returned.
   *
   ...
   * <code>toKey</code> is not compatible with the map
   */
  @Override
  ...
  private void subMapRecurse(K fromKey, K toKey, Position<Entry<K, V>> p,
    ...
    if (isInternal(p)) {
      if (compare(p.getElement(), fromKey) < 0) // p's key is less than fromKey, so any relevant entries are to the right
      {
