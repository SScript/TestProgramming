        ...
        for (int i = 0; i < bundleItems.length(); i++) {
            ...
            try {
                ...
                if ("TELCO".equalsIgnoreCase(GetBaseData().getOrderedService())) {
                    ...
                    // laiþam offerOrders ciklâ visus TELCO, jâfiltrç tur
//                    addline = true;
                    ...
                    // SOABG-1402 // ðî daïa tiek izmantota ProcessJson failâ, ne-TELCO
                ...
                if (addline) {
                    ...
                    for (int j = 0; j < itempriccount; j++) {
                        ...
                        // ðeit ir dati no OrderOffers(Pricing daïa)
                        ...
                        // SOAIP-2576
                        // Ja OrderItem.OrderPricing.OCPPlanCode satur vairâkus OCP plânus - t.i. satur ',' -
                        // pievienot UNICORN izsaukumam OfferPrice katram OCP planam atseviðíi.
                        // Piemçrâm OrderItem.OrderPricing.OCPPlanCode = 'z.OCP.Plan1,z.OCP.Plan2',
                        // tad pirmais OfferPrice ieraksts bûs ar ORDER_LINE_PLAN_CODE = 'z.OCP.Plan1',
...
//                                System.out.println("skip this1 = " + GetJsonObjectStringValue(objOrdPricing, "Amount", false));
                                continue; // skipojam iterâciju, ejam uz nâkamo orderPricing objektu
                            }
                        }
                        ...
                        // SOAIP-2600
                        // Sûtit visus orderPricing ierakstus, kuriem OrderItem.OrderPricing.Source = 'Promotion'
                        // un kas ir zem OrderItem ar aizpildîtu TetAdjustmentSource, visiem servisiem,
                        // kas ir zem offer ar to paðu RootOrderItemId
        ...
        for (int i = 0; i < itemcount; i++) {
            ...
            try {
                ...
                if ("Remove".equalsIgnoreCase(orderedServiceType)) {
                    ...
                    if ("Plan".equalsIgnoreCase(prdtSubType)) {
                        // jâtiek klât pie OrderPricing, lai salîdzinâtu vai Action ir Add, skatît dok.
